...
func Test_EnhancedAdvisor_combinedDomainStats(t *testing.T) {
	...
	priority.GetInstance().AddWeight("machine", 9000)
	...
	tests := []struct {
		...
		{
			name: "three groups with two weight",
			domainsMon: &monitor.DomainStats{
				Incomings: map[int]monitor.GroupMBStats{
					0: {
						"dedicated-60": map[int]monitor.MBInfo{
	...
	for _, tt := range tests {
		...
		t.Run(tt.name, func(t *testing.T) {
			t.Parallel()
			d := &EnhancedAdvisor{}
			gotStats, gotGroupInfo, err := d.combineDomainStats(tt.domainsMon)
			if (err != nil) != tt.wantErr {
				t.Errorf("combineDomainStats() error = %v, wantErr %v", err, tt.wantErr)
				return
			}
			if !reflect.DeepEqual(gotStats, tt.wantStats) {
				t.Errorf("combineDomainStats() gotStats = %v, want %v", gotStats, tt.wantStats)
			}
			if !reflect.DeepEqual(gotGroupInfo, tt.wantGroupInfo) {
				t.Errorf("combineDomainStats() gotGroupInfo = %v, want %v", gotGroupInfo, tt.wantGroupInfo)
			}
		})
	}
}
