...
// advisor is able to work with resctrl groups of identical priority as if a logical group,
// by leveraging the basicAdvisor working with groups of distinct priorities only.
...
func (a *advisor) GetPlan(ctx context.Context, domainsMon *monitor.DomainStats) (*plan.MBPlan, error) {
	domainStats, groupInfos, err := a.combinedDomainStats(domainsMon)
	if err != nil {
		return nil, err
	}
	mbPlan, err := a.basic.GetPlan(ctx, domainStats)
	if err != nil {
		return nil, err
	}
...
func (a *advisor) combinedDomainStats(domainsMon *monitor.DomainStats) (*monitor.DomainStats, *groupInfo, error) {
...
func (a *advisor) splitPlan(mbPlan *plan.MBPlan, groupInfos *groupInfo) *plan.MBPlan {
