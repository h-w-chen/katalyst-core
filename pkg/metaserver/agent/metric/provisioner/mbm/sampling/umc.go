	...
	for skt := 0; skt < m.SysInfo.MachineInfo.NumSockets; skt++ {
		for umc := 0; umc < m.PMU.UMC.NumPerSocket; umc++ {
			...
			if (umc+1)%m.PMU.UMC.NumPerPackage == 0 {
				...
				m.MemoryBandwidth.Packages[pkgIdx].RMB_Delta = mbw.Delta(48, cas_rd, m.MemoryBandwidth.Packages[pkgIdx].RMB)
				m.MemoryBandwidth.Packages[pkgIdx].WMB_Delta = mbw.Delta(48, cas_wr, m.MemoryBandwidth.Packages[pkgIdx].WMB)
				m.MemoryBandwidth.Packages[pkgIdx].Total = mbw.PMUToMB(m.MemoryBandwidth.Packages[pkgIdx].WMB_Delta+m.MemoryBandwidth.Packages[pkgIdx].RMB_Delta, m.Interval)
