...
import (
	...
	msr "github.com/kubewharf/katalyst-core/pkg/util/mbw/msr"
...
const (
	...
	MEMORY_BANDWIDTH_INCREASE_MIN            = 200    // the minimum incremental value
	MEMORY_BANDWIDTH_DECREASE_MIN            = 500    // the minimum decremental value
...
func checkMSR(core uint32, msReg int64, target uint64) error {
	ret, err := msr.ReadMSR(core, msReg)
	if err != nil {
	...
	if ret != target {
		return fmt.Errorf("failed to set msr %d on core %d to the expected value %d from %d",
			msReg, core, target, ret)
	}

	return nil
}

func writeMBAMSR(core uint32, msReg int64, val uint64) error {
	if wErr := msr.WriteMSR(core, msReg, val); wErr != nil {
		general.Errorf("failed to write %d to msr %d on core %d - %v", val, msReg, core, wErr)
		return wErr
	}

	if cErr := checkMSR(core, msReg, val); cErr != nil {
	...
	for _, ccd := range m.SysInfo.NumaMap[node] {
		...
		if entry.Used {
			...
			case MEMORY_BANDWIDTH_CONTROL_REDUCE:
				// workaround based on the experience: 1) MBA setting = current_MB - quota / 2
				if entry.Cap == 0 {
					entry.Cap = ccdMB*11/10 - ccdQuota/2
				} else {
		...
		if p.Total >= uint64(float64(m.Controller.MBThresholdPerNUMA)*m.Controller.PainPoint) {
			...
			if len(highPrisNumas) > 0 {
				if len(lowPrisNumas) > 0 && !m.Controller.NumaLowPriThrottled[i] {
					...
					quota := uint64(float64(p.Total-m.Controller.MBThresholdPerNUMA) * 0.5)
					...
					for _, node := range sortedHighPrisNumas {
						weight := float64(m.MemoryBandwidth.Numas[node].Total) / float64(aggregatedMB)
						...
						if m.MemoryBandwidth.Numas[node].Total <= (avgMB - deduction) {
			...
			if p.Total <= uint64(float64(m.Controller.MBThresholdPerNUMA)*m.Controller.UnthrottlePoint) {
				...
				// release throttling
				quota := uint64(float64(m.Controller.MBThresholdPerNUMA-p.Total) * 0.3)
				...
				for _, node := range sortedHighPrisNumas {
					if m.Controller.NumaThrottled[node] {
						weight := float64(m.MemoryBandwidth.Numas[node].Total) / float64(aggregatedMBThrottled)
