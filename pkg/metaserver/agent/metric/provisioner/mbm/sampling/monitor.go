...
import (
	...
	"github.com/kubewharf/katalyst-core/pkg/util/mbw"
	"github.com/kubewharf/katalyst-core/pkg/util/mbw/pci"
)
...
func newSysInfo() (*SysInfo, error) {
	...
	/*	hard code for the a tmp test - mock the case of 2 physical NUMA and 3 fake NUMA each */
	...
	sysInfo.CCDMap, err = mbw.GetCCDTopology(sysInfo.NumNUMANodes)
	if err != nil {
...
func (m MBMonitor) GlobalStats(ctx context.Context, refreshRate uint64) error {
	...
	return mbw.TickUntilDone(ctx, refreshRate, func() error {
...
func (m MBMonitor) ServeCoreMB() error {
	...
	for i := 0; i < len(m.MemoryBandwidth.Cores); i++ {
		...
		m.MemoryBandwidth.Cores[i].LRMB_Delta = mbw.Delta(24, coreMBMap[CORE_MB_READ_LOCAL][i], m.MemoryBandwidth.Cores[i].LRMB)
		...
		m.MemoryBandwidth.Cores[i].TRMB_Delta = mbw.Delta(24, coreMBMap[CORE_MB_READ_TOTAL][i], m.MemoryBandwidth.Cores[i].TRMB)
	...
	for i := 0; i < m.NumNUMANodes; i++ {
		...
		m.MemoryBandwidth.Numas[i].Total = mbw.RDTEventToMB(m.MemoryBandwidth.Numas[i].TRMB+uint64(wrRatio*float64(m.MemoryBandwidth.Numas[i].TRMB)), m.Interval, uint64(m.RDTScalar))
	}
